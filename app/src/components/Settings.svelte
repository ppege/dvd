...
<div class="flex flex-col">
	{#if visible}
		<div transition:fly={{ y: -200, duration: 200 }}>
			<div
				class="bg-black/25 md:bg-slate-800/60 md:rounded-br-2xl px-2 py-2 flex flex-col md:flex-row w-screen md:w-auto items-center md:items-start"
			>
				{#if !$previewMode}
					<MediaQuery query="(min-width: 768px)" let:matches>
						{#if matches}
							<Tab1 on:message={() => dispatch('message')} />
							<Tab2 />
						{:else}
							<TabGroup class="justify-center items-center content-center">
								...
								<TabPanels>
									<TabPanel>
										<Tab1 center on:message={() => dispatch('message')} />
									</TabPanel>
									<TabPanel>
										<Tab2 center />
									</TabPanel>
			...
			<button
				class="w-10 h-10 py-1 px-2 bg-black/50 {visible
					? 'opacity-100 hover:bg-black'
					: 'opacity-20'} hover:opacity-100 rounded-br-2xl"
	...
	{#if !visible && browser}
		<button
			class="w-10 h-10 py-1 px-2 z-50 bg-black/50 {visible
				? 'opacity-100 hover:bg-black'
				: 'opacity-20'} hover:opacity-100 absolute rounded-br-2xl"
