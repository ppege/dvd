<script lang="ts">
	...
	const generateShareCode = () => {
		...
		axios
			.post(`https://api.toof.rs/v2/postData`, localStorage)
			.then((response: any) => {
				if (!response.data.error) {
					...
					if (response.errors[0] === 'Please provide valid JSON') {
						notifyError({
							message: 'Tip: your URLs may include some bad characters like quotes or semicolons.',
							type: 'warning'
						});
					}
				}
	...
	const loadShareCode = () => {
		axios
			.get(`https://api.toof.rs/v2/dvd?code=${shareCode}`)
			.then((response) => {
	...
	const copyShareCode = () => {
		navigator.clipboard.writeText('https://dvd.toof.rs/#' + shareCode);
	...
	const savePreset = () => {
		...
		if (
			$presets.options.includes(name) &&
			!confirm('A preset with this name already exists. Overwrite?')
		) {
			return;
		}
...
{#if !$previewMode}
	<div class={`flex flex-col w-auto ${center ? 'items-center justify-center' : ''}`}>
		<div class="flex flex-col px-2 py-2 text-white items-center">
			<p>Share Code</p>
			<input
				class="text-input w-full text-black"
				type="text"
				bind:value={shareCode}
				on:keyup={debounceValidity}
			/>
			<div class="flex flex-row w-full mt-1">
				<button
					class="bg-white/10 hover:bg-white/25 rounded-tl border text-white px-2 py-1 w-2/3"
					on:click={generateShareCode}
				>
					<p>Generate</p>
				</button>
				<button
					class="bg-white/10 hover:bg-white/25 rounded-tr border text-white px-2 py-1 disabled:bg-black/5 disabled:border-slate-700 disabled:text-slate-300 w-2/3"
					on:click={copyShareCode}
					disabled={!shareCodeValid}
				>
					<p>Copy link</p>
				</button>
			</div>
			<div class="flex flex-row w-full">
				<button
					class="bg-white/10 hover:bg-white/25 rounded-bl border text-white px-2 py-1 disabled:bg-black/5 disabled:border-slate-700 disabled:text-slate-300 w-2/3"
					on:click={previewShareCode}
					disabled={!shareCodeValid}
				>
					<p>Preview</p>
				</button>
				<button
					class="bg-white/10 hover:bg-white/25 rounded-br border text-white px-2 py-1 disabled:bg-black/5 disabled:border-slate-700 disabled:text-slate-300 w-1/3"
					on:click={loadShareCode}
					disabled={!shareCodeValid}
				>
					<p>Load</p>
				</button>
		...
		<div class="flex flex-col px-2 py-2 items-center">
			...
			<select
				class="text-input font-sans w-full"
				bind:value={$presets.selected}
			...
			<div class="flex flex-row w-auto">
				<button
					...
					on:click={savePreset}>Save Preset</button
				>
				<button
					class="bg-white/10 hover:bg-white/25 border text-white px-2 py-1"
					on:click={deletePreset}>Delete Preset</button
				>
			</div>
		</div>
	</div>
{:else}
	<div class="flex flex-col text-white">
		...
		<button
			class="bg-white/10 hover:bg-white/25 rounded border text-white px-2 py-1 disabled:bg-black/5 disabled:border-slate-700 disabled:text-slate-300 w-full"
